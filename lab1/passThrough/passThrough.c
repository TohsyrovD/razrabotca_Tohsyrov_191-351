...
--*/
{
    ...
    //status = FltParseFileNameInformation(NameInfo); // Лог слишком большой, загружает ВМ
    //if (!NT_SUCCESS(status)) {
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            DbgPrint("Write");//индикатор, что обнаружен перехват события "запись"
        ...
        //проверка на событие IRP_MJ_READ
            ...
            DbgPrint("Read");//индикатор, что обнаружен перехват события "read"
